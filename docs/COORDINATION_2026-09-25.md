...
## 10. 游戏人物风格转换工作流解耦（2026-09-26）

实际使用反馈确认：页面信息量不是主要问题，问题在于高意图入口把用户带进了通用工作台。此前游戏人物案例页和首页案例卡都指向普通 `enhance` 工作流，上传、九种模式、案例展示、分享奖励和历史记录同时出现，用户需要自己判断下一步。

本轮将这条路径改成独立的 `?tool=game-character-style` 工作流：

- 游戏人物案例页、首页人物案例卡和静态首页 fallback 都直接进入风格转换入口，不再落到普通增强模式。
- 工作台在该查询下自动选择 edit 模式，并预填一份“保持轮廓、服装、动作、镜头和身份，只改变视觉变量”的转换 brief。
- 提供赛博朋克霓虹、写实奇幻、硬表面科幻、动漫赛璐璐和电影写实五个风格方向；选择方向会替换 brief，用户仍可在生成前编辑。
- 聚焦工作流只显示三步提示、风格设置和上传/结果区域；隐藏分享奖励、历史记录和右侧案例墙，普通 `/dashboard` 仍保留完整工具工作台。
- 提交按钮、标题、说明和中文界面均明确写出“风格转换”，让用户知道当前操作会保留人物并改变风格，而不是普通放大或修复。

验证结果：`npm run lint` 通过；`npm test` 72/72；`npm run build` 通过。构建后的 `/game-character-style` 和首页静态入口均指向 `/dashboard?tool=game-character-style`；本地浏览器复核聚焦工作流显示风格转换标题、五个方向和预填 brief，分享、历史与案例墙不出现。
