...
## 9. 生产收口与线上复核（2026-09-26）

网络恢复后，4 个本地提交已通过无代理方式推到 `origin/main`，当前生产提交为 `6d5c02c`：

- `62c3619`：风格工作流免登录首跑、样例入口与工作流本地化；
- `d7f54d4`：记录本地验收证据；
- `0623fff`：让通用编辑与风格工作流分别持有 brief / 文件状态，并把可运行样例放到游客首屏；
- `6d5c02c`：所有上传上限文案改为从 `MAX_UPLOAD_MIB` 派生，移除 2 MB / 5 MB 的过期承诺。

Vercel 生产部署已 Ready，线上浏览器复核结果如下：

- `/game-character-style` 能看到“先免费试一次：赛博朋克方向”、免费试用按钮，以及 20 组案例；
- `/dashboard?tool=game-character-style&sample=characterStyle` 能打开“DLSS5 风格转换工作流”，自动选中角色风格样例，显示 5 个风格方向单选项；
- 中文会话下工作室标题、设置、模式名、示例入口和按钮均为中文，复核到的工作室 chrome 没有英文残留；
- 风格深链下分享与历史入口按设计隐藏，避免把用户带回通用工作室；
- 首页上传入口已改为真实的工作室入口卡，所有可见上传大小文案统一由 `MAX_UPLOAD_MIB` 生成。

本地三层验证仍保持通过：`npm run lint`、`npm test`（72/72）和 `npm run build`。共享验收脚本需要在生产环境跑一次，尤其是首次免费样例会经历 202 → 轮询 → 缓存；这一步不重复触发真实生成，留给部署后的复测记录。

## 10. 线上复验与一次上游故障（2026-09-26 19:30）

第 9 节的部署是对的：用同一个验收脚本对 `https://www.dlss5nvidia.com` 跑 **5/5 通过**（今天早上同一脚本是 0/5）—— 首页诚实入口、`/game-character-style` 的 CTA 带 `&sample=characterStyle`、免登录示例按钮、风格深链打开正确工作流、zh 会话无英文 chrome。

但**免登录示例的第一次真实运行在生产上失败**，而且原因不在我们这一侧：

- 点击后请求 `POST /api/image-edit/samples {"sample":"characterStyle"}` 返回 `500 {"error":"Example generation failed"}`；
- 同一个接口的 `sample1`、`teapot` 仍返回 `cached: true`（**来自缓存**，所以看起来正常）；
- 用仓库自己的 provider 客户端在本地直连复现，`alphanet-flux`（生成式精修，即"提示词编辑/生成"这条线）对**任何**输入都返回 `FAILURE`，`fail_reason` 是上游自己塞的字符串 `"[object Object]"`，拿不到原文。对照用的一模一样的厨房样图与提示词（就是 sample1 当年跑出缓存的那组）现在也失败；
- 同一时间工具线是好的：`cutout-fast` 用一张全新图片跑 **SUCCESS**。

也就是说：**不是新样例的参数问题，是上游精修通道整体故障**。影响面比这一页更大 —— 任何登录用户点"生成"走的都是这条线。两件事需要跟进：
