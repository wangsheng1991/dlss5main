    ...
    <main lang={tool.locale} className="pt-28 sm:pt-32 pb-20 px-5 max-w-[1200px] mx-auto w-full">
      ...
      <section className="mt-16 grid grid-cols-1 md:grid-cols-2 gap-6" id="examples" aria-labelledby="use-cases-heading">
        ...
        <div className="rounded-xl border border-outline-variant/20 bg-surface-low p-6 sm:p-7">
          ...
          <p className="mt-5 text-xs leading-relaxed text-zinc-400">{es ? 'Estas herramientas utilizan la misma mejora de imagen con IA. No hay un modelo independiente de eliminación de desenfoque.' : 'These tools share the same AI enhancement mode. The unblur page uses this enhancement capability, not a separate deblurring model.'}</p>
        </div>
      </section>

      <section className="mt-16" aria-labelledby="steps-heading">
        <h2 id="steps-heading" className="text-3xl font-headline font-bold text-white mb-6">{es ? 'Cómo mejorar tu imagen' : 'How to enhance your image'}</h2>
      ...
      <section className="mt-16 border-t border-outline-variant/20 pt-8" aria-labelledby="related-heading">
        ...
        <a className="inline-block mt-6 text-sm text-primary" href="/blog/dlss-5-online-image-upscaler-guide">{es ? 'Guía sobre mejora de imágenes (en inglés) →' : 'Read the online image enhancement guide →'}</a>
      </section>
    </main>
  </>;
}
