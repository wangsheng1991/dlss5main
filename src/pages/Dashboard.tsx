...
import {
  ...
  MAX_UPLOAD_BYTES, failureNote, oversizeNote,
...
export default function Dashboard() {
  ...
  const choose = (candidate?: File) => {
    ...
    image.onload = () => {
      ...
      const tooBig = oversizeNote(image.naturalWidth, image.naturalHeight);
      ...
      {history.length === 0
        ...
        : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{history.map(job => <article key={job.id} className="rounded-lg border border-outline-variant/20 p-3 flex flex-col gap-3">
          {job.status === 'SUCCEEDED' && historyToken
            ...
            : <div className="aspect-video rounded-lg bg-surface-lowest border border-outline-variant/20 flex items-center justify-center px-3 text-center text-xs text-zinc-500">{job.status === 'FAILED' ? `Failed${job.errorCode ? ` · ${failureNote(job.errorCode)}` : ''} · credit refunded` : job.status.toLowerCase()}</div>}
