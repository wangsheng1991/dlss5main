      ...
      {profileHas('microTools') && (
        <section className="mt-32" aria-labelledby="small-tools-heading">
          <div className="mb-12 max-w-2xl">
            ...
            <p className="text-zinc-400 leading-relaxed">{isZh ? '免费证件照这类任务不需要生成式模型。上传、对齐规格，直接得到可下载的成品；图片只在本地浏览器处理，不上传也不扣积分。' : 'Free exact-format jobs need a finished file, not a model prompt. Upload, align the published specification and download it; the photo stays in your browser and uses no upload, account or credits.'}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {MICRO_TOOLS.map(item => <Link key={item.path} to={item.path} onClick={() => trackEvent('micro_tool_card_click', { tool: item.slug })} className="group rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/10 via-surface-low to-surface-low p-6 hover:border-primary transition-colors">
              <div className="flex items-start gap-4"><div className="w-12 h-12 shrink-0 rounded-xl bg-primary/15 flex items-center justify-center"><FileImage className="w-6 h-6 text-primary" /></div><div><p className="text-[10px] uppercase tracking-widest text-primary mb-2">Free local browser utility</p><h3 className="text-xl font-headline font-bold text-white group-hover:text-primary transition-colors">{item.heading}</h3><p className="mt-3 text-sm leading-relaxed text-zinc-400">{isZh ? '免费制作 3×4 或 35×45 mm 证件照，显示头部参考线，并导出单张 PNG 或可打印排版；照片留在本地。' : item.description}</p><span className="inline-block mt-5 text-sm text-primary font-semibold">{isZh ? '免费打开证件照工具 →' : 'Open the free passport photo maker →'}</span></div></div>
            </Link>)}
          </div>
        </section>
      )}
