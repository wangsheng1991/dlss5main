...
/** The studio's modes: two editing presets plus the tool lines, which size their own output. */
export type GenerationMode = 'edit' | 'enhance' | 'cutout' | 'vectorize' | 'erase' | 'tryon' | 'interior' | 'retouch' | 'makeup';
export interface GenerationBody {
  /** One or two uploaded file ids: the order is the contract of the tool they belong to. */
  image_ids: string[];
