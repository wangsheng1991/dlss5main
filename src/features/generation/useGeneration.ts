...
import { clearOperation, readOperation, saveOperation, terminal, type SavedOperation } from './operation';
...
export function useGeneration(user: User | null) {
  ...
  const query = useQuery({
    ...
    refetchInterval: query => terminal(query.state.data?.status) || query.state.status === 'error' || Date.now() - pollingStarted.current > 240000 ? false : 2000,
  ...
  async function submit(file: File, prompt: string) {
    ...
    try {
      ...
      const saved: SavedOperation = { version: 1, userId: user.uid, key: crypto.randomUUID(), body: { prompt: prompt.trim(), image_ids: [ticket.file_id], width: 1024, height: 1024, output_format: 'webp' }, createdAt: new Date().toISOString() };
  ...
  const waiting = pending && !!operation.jobId && !paused && !query.isError && Date.now() - pollingStarted.current <= 240000;
