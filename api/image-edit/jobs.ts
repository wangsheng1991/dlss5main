...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const claim = await store.claim(uid, idempotencyKey, { prompt: prompt.trim(), image_ids, width, height, seed, num_inference_steps, output_format });
    ...
    try {
      ...
      return res.status(202).json({ jobId: claim.id, status: 'QUEUED' });
    } catch (error) {
